...
const ShoppingCart = () => {
  ...
  const getItemCount = (cartVariantId: string) => {
    return (
      cartState.cartItems.find((item) => item.cartVariant.id === cartVariantId)
        ?.quantity || 0
    );
  };

  const handleCountChange = (quantity: number, cartVariantId?: string) => {
    const cartVariant = cartState.cartItems.find(
  ...
  return cartState.cartItems.length === 0 ? (
    ...
    <>
            ...
            <div className="flex flex-row items-center">
              ...
              <div className="ml-4 flex flex-col">
                ...
                <p className="text-sm md:text-base">
                  ${item.cartVariant.price.toFixed(2)}
                </p>
      ...
      <div className="flex w-full justify-between py-2">
        ...
        <p className="font-bold">{`$${cartState.totalPrice.toFixed(2)}`}</p>
      </div>
    </>
  );
};
